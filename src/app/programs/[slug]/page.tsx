...
import { getProgramBySlug, getProgramSlugs, programs } from "@/data/programs";
		...
		<main className="flex flex-col bg-[#FAF6F1]">
			<section className="bg-[#FDF2EE] px-4 py-12 sm:py-20">
				<div className="mx-auto max-w-5xl">
					<Link
						href="/programs"
						className="inline-flex items-center gap-2 text-sm font-semibold text-primary hover:text-[#8B3A24]"
					>
					...
					<div className="mt-6 flex items-start gap-2 text-sm font-medium text-[#2C1810]/70">
						<MapPin className="mt-0.5 h-4 w-4 shrink-0 text-primary" />
						<span>{program.location}</span>
					</div>
					<Link
						href={`/donate?program=${program.slug}`}
						className="mt-8 inline-flex h-11 items-center justify-center rounded-lg bg-[#E8A825] px-5 text-sm font-semibold text-[#2C1810] transition-colors hover:bg-[#F5D060]"
					>
						{program.donationCta}
					</Link>
				</div>
				...
				<div className="mx-auto grid max-w-6xl gap-6 lg:grid-cols-[1fr_0.9fr]">
					<div className="space-y-6">
						<Card className="rounded-lg bg-white shadow-sm">
							<CardHeader>
								<CardTitle className="text-2xl text-[#8B3A24]">
									Who benefits
								</CardTitle>
							</CardHeader>
							<CardContent>
						...
						<Card className="rounded-lg border-t-4 border-t-[#E8A825] bg-white shadow-sm">
							<CardHeader>
								<div className="flex items-center gap-2 text-[#8B3A24]">
									<Gift className="h-5 w-5 text-[#B8861E]" />
								...
								<div className="space-y-4">
									{program.giftUses.map((gift) => (
										<div key={gift.amount} className="border-l-2 border-[#E8A825] pl-4">
											<p className="font-semibold text-[#8B3A24]">{gift.amount}</p>
			...
			<section className="bg-[#F5EFE6] px-4 py-14">
			...
			<section className="px-4 py-14">
				<div className="mx-auto max-w-6xl">
					<h2 className="font-heading text-2xl font-bold text-[#8B3A24]">
						Other programs
					</h2>
